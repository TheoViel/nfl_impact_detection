...
def main():
    ...
    config.redundant_bias = (
        args.redundant_bias
    )  # redundant conv + BN bias layers (True to match official models)
    ...
    if args.resume:
        resume_state, resume_epoch = resume_checkpoint(
            _unwrap_bench(model), args.resume
        )
    ...
    dataset_train = CocoDetection(
        os.path.join(args.data, train_image_dir), train_annotation_path
    )
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                ema_eval_metrics = validate(
                    model_ema.ema, loader_eval, args, log_suffix=" (EMA)"
                )
        ...
        if use_amp:
            ...
            if args.clip_grad:
                torch.nn.utils.clip_grad_norm_(
                    amp.master_params(optimizer), args.clip_grad
                )
        else:
            loss.backward()
            if args.clip_grad:
