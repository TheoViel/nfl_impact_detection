...
def _post_process(config, cls_outputs, box_outputs):
    ...
    #######################################
    # INITIAL : RETRIEVE THE MOST LIKELY HELMETS & IMPACTS
    ...
    #######################################
    # RETRIEVE THE MOST LIKELY HELMETS.
    # _, cls_topk_indices_all = torch.topk(
    #     cls_outputs_all[:, :, 0], dim=1, k=MAX_DETECTION_POINTS
    # )
    # indices_all = cls_topk_indices_all
    # classes_all = cls_topk_indices_all % 1
    ...
    cls_outputs_all_after_topk_final = torch.gather(
        cls_outputs_all_after_topk, 2, classes_all.unsqueeze(2)
    )

    return (
        cls_outputs_all_after_topk_final,
...
class DetBenchEval(nn.Module):
    ...
    def forward(self, x, image_scales):
        ...
        class_out, prob_out, box_out, indices, classes = _post_process(
