...
def generate_detections(
    cls_outputs, prob_outputs, box_outputs, anchor_boxes, indices, classes, image_scale
):
    ...
    detections = torch.cat([boxes, probs], dim=1)
    if len(top_detection_idx) < MAX_DETECTIONS_PER_IMAGE:
        detections = torch.cat(
            [
                detections,
                torch.zeros(
                    (MAX_DETECTIONS_PER_IMAGE - len(top_detection_idx), detections.shape[1]),
